...
class WC_Gateway_Totalweb extends WC_Payment_Gateway {
	...
	function __construct() { 
		...
		$this->id			= 'totalweb';
                $this->icon 		= apply_filters('woocommerce_totalweb_icon', plugins_url('images/SupportedCardsMain.png', __FILE__));
		...
		// add store card option at checkout
		add_filter( 'woocommerce_after_order_notes' , array(&$this, 'add_custom_checkout_field_storecard') );
                add_action( 'woocommerce_checkout_update_order_meta', array(&$this,'update_order_store_card_meta') );
    ...
    function add_custom_checkout_field_storecard( $checkout ) {

        woocommerce_form_field( 'tws_storecard', array(
            'type'          => 'checkbox',
            'checked'          => 'checked',
            'default'          => 1,
            'required'          => true,
            'class'         => array('my-field-class form-row-wide'),
            'label'         => 'Store Card Securely For Future Use',
            ), $checkout->get_value( 'tws_storecard' ));
    }

    function update_order_store_card_meta( $order_id ) {

        update_post_meta( $order_id, '_tws_storecard', sanitize_text_field( $_POST['tws_storecard'] ) );
    }
    ...
    function init_form_fields() {
    	...
    	$this->form_fields = array(
			...
			'debug' => array(
						...
						'type' => 'checkbox', 
						'label' => __( 'Enable logging (<code>' . wc_get_log_file_path( 'totalweb' ).  '</code>)', 'woocommerce' ), 
						'default' => 'no'
					)
			);
    }
...
<?php
	...
	function get_params( $order) {
		...
		$hn = '';
			...
			$re2 = '/^.*?(?=\d)/';
			$re3 = '/^(.+?),/';
			$str = $order->billing_address_1;



			if (preg_match_all('/^Unit/i', $str, $housenumber, PREG_SET_ORDER, 0)) {

				$fre = '/((?:[a-zA-Z]+\s)*)([\d]+[a-z]?)(?:(?:\/)(\d+))?/i';

				preg_match_all($fre, $str, $flatn, PREG_SET_ORDER, 0);

				$hn = $flatn[0][0];

			}
			elseif (preg_match_all('/^Flat/i', $str, $housenumber, PREG_SET_ORDER, 0)) {
			...
			elseif (preg_match_all('/^\d+/', $str, $housenumber, PREG_SET_ORDER, 0)){
		

				if (preg_match_all($re1, $str, $matches, PREG_SET_ORDER, 0)){
				...
				$re = '/^.*?(?=\d)/';
				$subst = '';
				$result1 = preg_replace($re, $subst, $strim, 1);
				$hn = $result1;
				}
		}

		elseif (preg_match_all($re3, $str, $matches, PREG_SET_ORDER, 0)){
		
			$hn = $matches[0][1]; 
		}

		elseif (preg_match($re2, $str, $matches, PREG_OFFSET_CAPTURE, 0)){
		...
		/* see if customer has conesent to store card details for future use */	
                $storecard = $order->get_meta( '_tws_storecard', true );

  	        if ($storecard == 1) {
		   $transactionidentity = "CIT";
                } else {
		   $transactionidentity = "NNN";
		}	
...
<form action="<?php echo $pay_url ?>" method="post" id="totalweb_payment_form">
	...
	<input type="submit" class="button button-alt" id="submit_totalweb_payment_form" value="<?php _e('Pay via card', 'woocommerce') ?>" /> 
...
<script type="text/javascript">					
	jQuery(function($){
        ...
        $("#totalweb_payment_form").attr("enctype" , "application/x-www-form-urlencoded") ;
...
<?php
	...
	function receipt_page( $order ) {
		echo '<p>'.__('Thank you for your order, please wait to pay with Credit or Debit card.', 'woocommerce').'</p>';
	...
	public function response_handler() {
		...
		if (isset($_GET['authorizeListenerSIM'])) {
			...
			if($hdl == 'relay') {
				...
				$this->log( "Relay response:" . print_r($_POST,true));
				$this->log( "Login ID:" . $this->login_id . "; md5: " . $this->md5_hash);
				...
				if ($response->isAuthorizeNet()) {
					if ($response->approved) {
						...
						if(!empty($order_id)) {
							...
							$this->log( 'Total Web Solutions payment completed (Transaction ID: ' . $response->transaction_id . ')');
						...
						$this->log( sprintf("Error %s: %s", $response->response_reason_code, $response->response_reason_text));
						
						$redirect = add_query_arg('authorizeListenerSIM', 'error', $this->notify_url);
						$redirect = add_query_arg('reason_code', $response->response_reason_code, $redirect); // add reeson code
						$redirect = add_query_arg('reason_text', __("Payment Failed. Please check your details and try again", "woocommerce"), $redirect); // add reeson text
						$redirect = add_query_arg('code', $response->response_code, $redirect); // add error code
						
					}

				} else {
					$this->log( "MD5 Hash failed. Check to make sure your MD5 Setting matches the one in admin option");
					
					$redirect = add_query_arg('authorizeListenerSIM', 'error', $this->notify_url);
					$redirect = add_query_arg('reason_text', __("MD5 Hash failed. Check to make sure your MD5 Setting matches the one in admin option", "woocommerce"), $redirect);
				}
